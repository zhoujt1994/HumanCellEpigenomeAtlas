...
|---|---|---|
| **Compartments** (`comp.raw.hdf`, `comp.impute.hdf`, `merged_comp.hdf`, dcHiC `DifferentialResult`) | `fig4/08.compartment`, `fig4/09.diffcomp_majortype` | cell-type `Q.cool` (100 kb) → **cworld/dcHiC** |
| **Loop hdfs** (`loop_Q.hdf`, `loop_T.hdf`, `merged_loop.hdf`) | `fig4/05.diffloop_majortype` — concat `{ct}.loop.bedpe` → `merged_loop`, then `load_Q` reads Q/T at each loop anchor from `{ct}.{Q,T}.cool` (cache-guarded `to_hdf`) | the provided `{ct}.loop.bedpe` + `Q/T.cool` |
